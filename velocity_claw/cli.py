...
from velocity_claw.release_notes import write_release_notes
...
def release_readiness_cli(as_json: bool = False):
    result = ReleaseReadinessEvaluator(load_settings()).evaluate()
    _print_payload(result, as_json=as_json)


def validate_package_cli(as_json: bool = False):
    _print_payload(validate_package(Path.cwd()), as_json=as_json)


def generate_release_notes_cli(as_json: bool = False):
    output_path = write_release_notes(Path.cwd())
    _print_payload({"status": "ok", "path": str(output_path)}, as_json=as_json)
...
def list_runs_cli(limit: int, as_json: bool = False):
    agent = build_agent()
    _print_payload({"runs": agent.memory.list_recent_runs(limit=limit)}, as_json=as_json)
...
def retry_context_cli(run_id: str, as_json: bool = False):
    _print_payload(build_agent().build_retry_context(run_id), as_json=as_json)
...
def run_bot():
    ...
    VelocityClawTelegramBot(settings=load_settings()).run()
...
def main():
    ...
    if args.server:
        ...
        memory_cleanup_cli(days=args.memory_retention_days, keep_min_runs=args.memory_keep_min_runs, vacuum=not args.memory_no_vacuum, as_json=args.json)
    elif args.runs:
        ...
        _print_payload(build_agent().get_status(), as_json=args.json)
    else:
        parser.print_help()
