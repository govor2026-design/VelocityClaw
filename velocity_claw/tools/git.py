...
class GitTool:
    def __init__(self, settings: Settings):
        ...
        self.safe_commands = {
            ...
            "show", "stash", "stash pop", "stash list", "pull", "fetch"
        }
    ...
    def validate_git_command(self, command: str) -> List[str]:
        ...
        subcommand = " ".join(args[1:2]) if len(args) > 1 else ""
        full_subcommand = " ".join(args[1:]) if len(args) > 1 else ""

        if subcommand not in self.safe_commands and full_subcommand not in self.safe_commands:
    ...
    def validate_repo_root(self, cwd: str = None) -> Path:
        """Ensure cwd is within repo_root."""
        if cwd is None:
