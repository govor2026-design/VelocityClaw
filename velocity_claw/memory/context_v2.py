...
class ProjectContextV2:
    ...
    Run history and operational notes remain in the existing memory tables. Reusable
    knowledge is stored separately so short-lived execution trace does not dominate
    ...
    def _reusable_notes(self, limit: int) -> tuple[list[dict[str, Any]], int]:
        ...
        for note in notes:
            ...
            if note_type in TRACE_NOTE_TYPES:
                ignored_trace += 1
                continue
            if note_type not in REUSABLE_NOTE_TYPES or not content:
                continue
    ...
    def _related_runs(self, task: str, limit: int) -> list[dict[str, Any]]:
        candidates = self.memory.list_recent_runs(limit=100)
    ...
    def _knowledge_relevance(self, task: str, entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for entry in entries:
            value_text = _safe_json(entry.get("value"))
            entry_tokens = normalize_tokens(f"{entry.get('key')} {entry.get('category')} {value_text}")
    ...
    def build(self, task: str, *, limit: int = 5) -> dict[str, Any]:
        ...
        structured = self._knowledge_relevance(task, self.list(limit=100))[:bounded_limit]
        ...
        legacy_facts = self.memory.list_project_facts()
        return {
            "context_version": 2,
            "task": task,
            "reusable_knowledge": structured,
            "legacy_project_facts": legacy_facts,
